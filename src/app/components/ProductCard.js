...
const ProductCard = ({ product }) => {
  ...
  return (
    <>
      <div
        className="bg-gray-800 w-full rounded-xl shadow-md hover:shadow-lg transition-shadow duration-300 overflow-hidden sm:max-w-sm sm:mx-auto border border-gray-700"
        // data-aos={product?.dataos}
      >
        {/* Product Image Container (click to open) */}
        <div
          className="relative h-78 w-full bg-gray-900 max-md:h-[550px] cursor-pointer"
          onClick={() => setIsOpen(true)}
          role="button"
          aria-label={`عرض صورة ${product.title}`}
        >
          <Image
      ...
      {isOpen && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label={`تكبير صورة ${product.title}`}
        >
          <div
